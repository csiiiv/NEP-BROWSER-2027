"""
NEP-FY2026 streaming converter.
...
and produces:
  ...
  data/budget/2026/
    budget-mapping.json
    items/nep_2026_batch_XXXX.json.gz   (~100k records per batch, gzipped)
...
Usage:
    python converter_fy2026.py                 # full run
    python converter_fy2026.py --limit 5000    # dry run on first 5000 rows
    python converter_fy2026.py --input PATH    # custom intermediate JSON path
"""
...
class FY2026Converter:
    """Streaming converter and reference extractor for NEP-FY2026."""
    ...
    def _add_operating_unit(
        ...
        # OPERUNIT in NEP-FY2026 is 7 digits (class + lower_ou). Pad if short.
    ...
    def _add_region(self, region_raw: str, description: str) -> Optional[str]:
        ...
        if region == "00" or not region.isdigit():
            ...
            return None
        # NEP-FY2026 does not carry UACS_REG_DSC -- fall back to the canonical
        # region names from the PSGC/UACS reference (sourced from FY2027's
        # UACS_REG_DSC, which is identical to DBM's region table).
        if not description:
            description = _REGIONS_FALLBACK.get(region, "")
    ...
    def _add_division(self, operdiv_raw: str, description: str) -> tuple:
        ...
        # Prefer a non-empty description if a later row fills it in.
    ...
    def _write_reference_jsons(self) -> None:
        ...
        _write_json(self.org_dir / "_metadata.json", [{
            ...
            "source": "derived from NEP-FY2026",
        ...
        _write_json(self.funding_dir / "_metadata.json", [{
            ...
            "source_funding_sources": "derived from NEP-FY2026",
            "conversion_date": datetime.now().isoformat(),
        }])
        ...
        _write_json(self.location_dir / "_metadata.json", [{
            ...
            "total_barangays": 0,
            "note": "NEP-FY2026 only carries region-level geography",
            "source": "derived from NEP-FY2026",
            "conversion_date": datetime.now().isoformat(),
        }])
...
# Region code -> description fallback. NEP-FY2026 lacks the UACS_REG_DSC
# column present in FY2027, so we hardcode the canonical DBM/PSGC region
# names (identical across years). Sourced from FY2027's UACS_REG_DSC.
_REGIONS_FALLBACK = {
    "01": "Region I - Ilocos",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="NEP-FY2026 streaming converter")
    here = Path(__file__).resolve().parent
    default_input = here / "input" / "NEP-FY2026.json"
    ...
    converter = FY2026Converter(args.input, args.output)
